  ...
  private static string shoppingCartId
  {
    get
    {
      ...
      // if the cart ID isn't in the cookie...
          if (context.Request.Cookies["ShoeShop_CartID"] != null)
        {
          // return the id
            return context.Request.Cookies["ShoeShop_CartID"].Value;
        }
        else
        // if the cart ID doesn't exist in the cookie as well, generate a new ID
        {
          ...
          HttpCookie cookie = new HttpCookie("ShoeShop_CartID", cartId);
          // set the cookie's expiration date
          int howManyDays = ShoeShopConfiguration.CartPersistDays;
